...
const indivIns = testData.split(',');

const indivIns2 = testData2.split(',');

let firstCoord = { x: 0, y: 0 };

let firstCoord2 = { x: 0, y: 0 };

const createCoord = (x, y, s) => {
  return {x: x, y: y, steps: s};
}
...
indivIns.forEach(ins => {
  const numberOfIts = ins.slice(1);
  if (ins[0] === 'U' || ins[0] === 'D') {
    const isAdd = ins[0] === 'U' ? true : false;
    let newCoord, coOrdToAdd;
    for(i=0; i<numberOfIts; i++) {
      if (isAdd) {
        newCoord = firstCoord.y + 1;
        coOrdToAdd = createCoord(firstCoord.x, newCoord, steps1);
        firstLine.push(coOrdToAdd)
        firstCoord = {x: firstCoord.x, y: newCoord};
        steps1++;
      } else {
        newCoord = firstCoord.y - 1;
        coOrdToAdd = createCoord(firstCoord.x, newCoord, steps1);
    ...
    const isAdd = ins[0] === 'R' ? true : false;
    for(j=0; j<numberOfIts; j++) {
      if (isAdd) {
        newCoord = firstCoord.x + 1;
        coOrdToAdd = createCoord(newCoord, firstCoord.y);
        firstLine.push(coOrdToAdd)
        firstCoord = {x: newCoord, y: firstCoord.y, steps1};
        steps1++;
      } else {
        newCoord = firstCoord.x - 1;
        coOrdToAdd = createCoord(newCoord, firstCoord.y, steps1);
        firstLine.push(coOrdToAdd)
        firstCoord = {x: newCoord, y: firstCoord.y};
        steps1++;
      }
    }
  }
...
indivIns2.forEach(ins => {
  const numberOfIts2 = ins.slice(1);
  if (ins[0] === 'U' || ins[0] === 'D') {
    const isAdd = ins[0] === 'U' ? true : false;
    for(i=0; i<numberOfIts2; i++) {
      if (isAdd) {
        const newCoord2 = firstCoord2.y + 1;
        const coOrdToAdd2 = createCoord(firstCoord2.x, newCoord2, steps2);
        secondLine.push(coOrdToAdd2)
        firstCoord2 = {x: firstCoord2.x, y: newCoord2};
        steps2++;
      } else {
        const newCoord2 = firstCoord2.y - 1;
        const coOrdToAdd2 = createCoord(firstCoord2.x, newCoord2, steps2);
    ...
    for(i=0; i<numberOfIts2; i++) {
      if (isAdd) {
        const newCoord2 = firstCoord2.x + 1;
        const coOrdToAdd2 = createCoord(newCoord2, firstCoord2.y, steps2);
        secondLine.push(coOrdToAdd2)
        firstCoord2 = {x: newCoord2, y: firstCoord2.y};
        steps2++;
      } else {
        const newCoord2 = firstCoord2.x - 1;
        const coOrdToAdd2 = createCoord(newCoord2, firstCoord2.y, steps2);
...
let winningCoords = {x: 0, y: 0};
firstLine.forEach(cd => {
  secondLine.forEach(scd => {
//     console.log(cd.x - scd.x)
    if(cd.x === scd.x && cd.y === scd.y) {
       if(cd.x + cd.y < lowestTotal ) {
         winningCoords = {x: cd.x, y: cd.y}
         lowestTotal = cd.x + cd.y;
       }
    }
  })
})

console.log('Closest point is, ', winningCoords);

let lowestSteps = 9999999;
let winningCoordsSteps = {x: 0, y: 0, s:0}
firstLine.forEach(cd => {
    secondLine.forEach(scd => {
  //     console.log(cd.x - scd.x)
      if(cd.x === scd.x && cd.y === scd.y) {
         if(cd.steps + scd.steps < lowestSteps ){
            const stepsTotal = cd.steps + scd.steps;
            winningCoordsSteps = {x: cd.x, y: cd.y, steps: stepsTotal}
           lowestSteps = cd.steps + scd.steps;
         }
      }
    })
  })

  console.log('Intersection with lowest steps is: ', winningCoordsSteps);
  console.log('lowest total steps is', lowestSteps);
